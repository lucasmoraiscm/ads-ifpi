...
class Poupanca extends Conta {
    ...
    public renderJuros() {
        let juros: number = this.saldo * this.taxaDeJuros / 100;
...
class Banco {
    ...
    public recuperarContasDeArquivo(nomeArquivo: string): boolean {
        const fileContent: string = fs.readFileSync(nomeArquivo, 'utf-8');
        ...
        for (let line of lines) {
            if (line != '') {
                ...
                let tipoConta: string = dadosLinha[2];

                switch(tipoConta) {
                    case 'C':
                        let conta: Conta = new Conta(dadosLinha[0], parseFloat(dadosLinha[1]));
                        this.inserirConta(conta);
                        break;
                    case 'CP':
                        let contaPoupanca: Poupanca = new Poupanca(dadosLinha[0], parseFloat(dadosLinha[1]), parseFloat(dadosLinha[3]));
                        this.inserirConta(contaPoupanca);
                        break;
                    case 'CI':
                        let contaImposto: ContaImposto = new ContaImposto(dadosLinha[0], parseFloat(dadosLinha[1]), parseFloat(dadosLinha[3]));
                        this.inserirConta(contaImposto);
                        break;
                    default:
                        return false;
                }
    ...
    public gravarContasEmArquivo(nomeArquivo: string): void {
        for (let conta of this.contas){
            if (conta instanceof Poupanca) {
                let dadosConta: string = `${conta.numero}; ${conta.saldo}; CP; ${conta.taxaDeJuros}\n`;
                fs.appendFileSync(nomeArquivo, dadosConta);
            }
            else if (conta instanceof ContaImposto) {
                let dadosConta: string = `${conta.numero}; ${conta.saldo}; CI; ${conta.taxaDesconto}\n`;
                fs.appendFileSync(nomeArquivo, dadosConta);
            }
            else {
                let dadosConta: string = `${conta.numero}; ${conta.saldo}; C\n`;
                fs.appendFileSync(nomeArquivo, dadosConta);
            }
        }
    }
...
export {Conta, Poupanca, Cliente, Banco};
